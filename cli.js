...
/** Display AVAX account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} AVAX balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'avax' && refund !== '0') {
    throw new Error('The AVAX purchase is supposted to be 0 for AVAX withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 43114 || netId === 43113) {
        console.log(`Transaction submitted through the relay. View transaction on Avalanche https://cchain.explorer.avax.network`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 43114 || netId === 43113) {
          console.log(`View transaction on Avalanche https://cchain.explorer.avax.network/tx/${txHash}/internal-transaction`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'avax', amount = '0.1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/AVAXSmashnado.json')).json()
  ...
  if (!(netId === 1 || netId === 43113)) {
    throw new Error('You did not connect to Avalanche network. Please make sure you connected to Avalanche Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Avalanche Mainnet.')
  erc20 = currency !== 'avax' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'avax', amount: '0.1' }
  //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account AVAX balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
